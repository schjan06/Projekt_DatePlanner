...
- **Abgrenzung [Optional]:** Der Prototyp enthält ein einfaches Login-, Registrierungs- und User-System mit E-Mail-Bestätigung, aber kein produktionsreifes Rollen-, Rechte- oder Account-Management. Es gibt kein Zahlungs- oder Buchungssystem, keine produktive Anbieterintegration, kein OAuth, keinen Passwort-Reset und kein Cloud-Storage. Die Kartenlösung nutzt OpenStreetMap/Leaflet statt einer kostenpflichtigen Google-Maps-API. Bilduploads für neue Aktivitäten werden prototypisch als kleine Data-URLs in MongoDB gespeichert. Produktive Datenschutz-, Rollen- und Rechtekonzepte sind TODO.
...
- **Login-Seite:** Die Login-Seite ist als fokussierte Einstiegskarte gestaltet. Sie enthält Branding, einen kurzen Nutzenhinweis, Benutzername-/E-Mail- und Passwortfelder sowie eine klare Login-Aktion. Demo-Zugangsdaten werden nicht mehr prominent angezeigt, damit die Seite sauberer wirkt.
...
- **Auth/Login:** Das Login-System nutzt `src/hooks.server.js`, `src/lib/server/auth.js`, MongoDB-Collections `users`, `sessions` und `emailVerificationTokens` sowie das Cookie `vm_session`. Passwörter werden mit Node.js `crypto.scrypt` gehasht. Neue Accounts werden über `/register` erstellt und können sich erst nach Bestätigung des E-Mail-Links unter `/verify-email?token=...` einloggen. Der E-Mail-Versand erfolgt per SMTP über `nodemailer` und `.env`-Konfiguration. Login ist mit Benutzername oder E-Mail möglich. Der Demo-Login lautet `demo` / `demo123`; der Demo-User ist ein normaler Seed-/Präsentationsaccount und wird nicht mehr zur Laufzeit als Fallback erzeugt. `passwordHash` wird nie ans Frontend gesendet.
...
- **Beschreibung & Nutzen:** Die App ist nur nach Login nutzbar. Neue Nutzer können sich registrieren, müssen ihre E-Mail-Adresse bestätigen und können danach auf die App zugreifen. Dadurch können Profil, Wishlist, geplante Aktivitäten und weitere nutzerbezogene Aktionen sauber einem User zugeordnet werden.
- **Wo umgesetzt:**
  - **Frontend:** `src/routes/login`, `src/routes/register`, `src/routes/verify-email`.
  - **Server/Auth:** `src/hooks.server.js`, `src/lib/server/auth.js`, `src/lib/server/email.js`, `src/routes/login/+page.server.js`, `src/routes/api/auth/login`, `src/routes/api/auth/logout`.
  - **Datenbank:** Collections `users`, `sessions` und `emailVerificationTokens`.
- **Technische Umsetzung:** Nach erfolgreichem Login wird ein zufälliges Session-Token erzeugt, gehasht in MongoDB gespeichert und als `httpOnly` Cookie `vm_session` gesetzt. `hooks.server.js` prüft die Session und schützt App-Routen. Passwörter werden mit `scrypt` gehasht. Bei der Registrierung wird ein neuer User mit `emailVerified: false` erstellt, ein Bestätigungstoken gehasht gespeichert und per SMTP-Mail verschickt. Erst nach erfolgreicher Verifikation wird `emailVerified: true` gesetzt.
- **Setup-Hinweis:** Für echten Mailversand müssen `APP_URL`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASS` und `SMTP_FROM` in `.env` gepflegt werden. Die nötigen Schlüssel stehen in `.env.example`.
...
- **Manuelle Testcheckliste aktueller Prototyp:**
  ...
  - Registrierung mit Benutzername, E-Mail und Passwort durchführen.
  - Vor E-Mail-Bestätigung einloggen: Login wird blockiert.
  - Bestätigungslink aus SMTP-Mail öffnen und danach Login mit Benutzername oder E-Mail prüfen.
